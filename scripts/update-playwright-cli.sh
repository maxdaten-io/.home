...
NIX_FILE="$REPO_ROOT/users/jloos/modules/playwright-cli.nix"
LOCK_FILE="$REPO_ROOT/users/jloos/modules/playwright-cli/package-lock.json"

# Get version from argument or fetch latest
if [[ $# -ge 1 && $1 == "latest" ]]; then
  echo "Fetching latest version from npm..."
  VERSION=$(npm view @playwright/cli version 2>/dev/null)
elif [[ $# -ge 1 ]]; then
  VERSION="$1"
else
  echo "Usage: update-playwright-cli.sh <version|latest>"
  exit 1
fi
...
SRC_HASH_RAW=$(nix-prefetch-url "https://registry.npmjs.org/@playwright/cli/-/cli-${VERSION}.tgz" 2>/dev/null)
...
npm install "@playwright/cli@${VERSION}" --package-lock-only 2>/dev/null
popd >/dev/null
...
echo ""
echo "==> Done! Updated playwright-cli: $CURRENT_VERSION -> $VERSION"
echo ""
echo "Changes:"
